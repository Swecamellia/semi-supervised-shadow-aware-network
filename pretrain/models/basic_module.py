...
from models.ops import conv2d_resample
from models.ops import upfirdn2d
from models.ops import bias_act
...
class StyleConv(torch.nn.Module):
    def __init__(self,
        ...
        up              = 1,            # Integer upsampling factor.     #原来是upsample的因子
        use_noise       = False,         # Enable noise input?    Original use_noise=True,我的想法是mask已经引起noise了，所以中间就不在加入noise了
...
class UpSampling2d(nn.Module):
    def __init__(self, in_channel, out_channel, kernel_size, stride=1, padding=0,
        ...
        if hold_mode == 'hold_first':
            hold_in_channels = in_channel
        elif hold_mode == 'hold_last':
            hold_in_channels = out_channel
        ...
        if hold_mode == 'kernel_conv':
            ...
            self.hold_conv = HoldConv2d(
                in_channels=hold_in_channels, zoh_kernel_size=stride, order=order,
                stride=1, padding=0, bias=False, padding_mode='zeros'
            )
            self.trans_conv = nn.ConvTranspose2d(
                in_channel, out_channel, kernel_size, stride=stride,
                padding=padding, output_padding=padding, dilation=dilation, groups=groups,
                bias=bias, padding_mode=padding_mode
            )
        elif bias_mode == 'bias_last':
            self.hold_conv = HoldConv2d(
                in_channels=hold_in_channels, zoh_kernel_size=stride, order=order,
                stride=1, padding=0, bias=bias, padding_mode='zeros'
            )
            self.trans_conv = nn.ConvTranspose2d(
                in_channel, out_channel, kernel_size, stride=stride,
                padding=padding, output_padding=padding,  groups=groups,
...
class DecBlockFirst(nn.Module):
    def __init__(self, in_channels, out_channels, activation, style_dim, use_noise, demodulate, img_channels):
        ...
        self.conv = StyleConv(in_channel=in_channels,
...
class DecBlockFirstV2(nn.Module):
    def __init__(self, in_channels, out_channels, activation, style_dim, use_noise, demodulate, img_channels):
        ...
        self.conv1 = StyleConv(in_channel=in_channels,
                              out_channel=out_channels,
                              style_dim=style_dim,
                              resolution=4,
                              kernel_size=3,
...
class DecBlock(nn.Module):
    def __init__(self, res, in_channels, out_channels, activation, style_dim, use_noise, demodulate, img_channels):  # res = 2, ..., resolution_log2
        ...
        self.conv0 = StyleConv(in_channel=in_channels,
                               out_channel=out_channels,
                               style_dim=style_dim,
                               resolution=2**res,
                               kernel_size=3,
        ...
        self.conv1 = StyleConv(in_channel=out_channels,
                               out_channel=out_channels,
                               style_dim=style_dim,
                               resolution=2**res,
                               kernel_size=3,
